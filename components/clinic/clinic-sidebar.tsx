...
import { FlaskConical, Home, Settings, Upload } from "lucide-react"
...
export const CLINIC_NAV = [
  ...
  { href: "/clinic/upload", label: "Upload report", icon: Upload },
  { href: "/clinic/reports", label: "Reports", icon: FlaskConical },
]

export function ClinicSidebar({ userEmail, logoUrl }: { userEmail: string; logoUrl?: string | null }) {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden h-svh w-64 flex-col border-r border-border bg-sidebar text-sidebar-foreground lg:flex">
      ...
      <div className="px-6 pt-4">
        <p className="text-xs uppercase tracking-wider text-muted-foreground">Diagnostic lab</p>
      </div>

      <nav className="flex-1 overflow-y-auto px-3 py-6" aria-label="Primary">
        <ul className="space-y-1">
          {CLINIC_NAV.map((item) => {
            const active = pathname === item.href || (item.href !== "/clinic" && pathname.startsWith(item.href))
            return (
              <li key={item.href}>
                <Link
                  href={item.href}
